document.addEventListener('DOMContentLoaded', function () {
  ...
  async function sendMessage(text) {
    ...
    try {
      ...
      const profil = {
        filiere_recommandee: top ? top.nom : null,
        matieres_preferees: profile ? [profile.matieres, profile.matiere_fav].filter(Boolean) : [],
        interets: profile ? [profile.secteur_ci, profile.activite].filter(Boolean) : [],
        budget_annuel_fcfa: profile ? profile.budget : null,
        ville_souhaitee: profile ? (profile.mobilite === 'abidjan' ? 'Abidjan' : 'Région') : null,
        duree_max_annees: profile ? profile.duree : null
      };

      const response = await fetch('/api/chat', {
        method: 'POST',
        headers: { 'Content-Type': 'application/json' },
        body: JSON.stringify({ message: text, profil })
      });
      ...
      appendMessage(data.reponse || 'Pas de réponse reçue.', 'bot');
